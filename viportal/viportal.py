...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="If set opens VS Code")

    parser.add_argument(
        '-e', '--explorer', action='store_const', const=True,
        help="If set opens explorer")

    parser.add_argument(
        '--list', action='store_const', const=True,
        help="Lists the availabe portals")
    ...
    if args.list is not None:
        ...
        if not args.name:
            ...
            if args.name not in config["portals"]:
                ...
                if args.explorer:
                    subprocess.call(f"explorer \"{portal['path']}\"", shell=True, env=my_env,
                                    creationflags=CREATE_NO_WINDOW)
